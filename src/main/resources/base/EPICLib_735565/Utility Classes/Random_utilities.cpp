...
{
    get_engine().seed(static_cast<Random_engine_t::result_type>(seed_));
}
...
int random_int(int range)
{
    ...
    int result = uid(get_engine(), parm_t(0, range - 1));
    Assert(result != range);
    return result;
}
...
double normal_random_variable(double mean, double sd)
{
    ...
    return nd(get_engine(), parm_t(mean, sd));
}
...
// a x increases, the probability that true is returned increases according to a Normal dbn from 0. to 1.0.
...
bool based_ogive_detection_function(double x, double base, double mean, double sd)
{
	...
	if(rv <= base)
		return true;
	return ogive_detection_function(x, mean, sd);
}
