...
// These differ only in whether a pitch parameter is included
...
Auditory_sound::Auditory_sound(const Symbol& name_, const Symbol& stream_, long time_stamp_, GU::Point location_, const Symbol& timbre_, 
			const Symbol& content_, const Symbol& gender_, const Symbol& speaker_id_, double loudness_, long duration_, long intrinsic_duration_) :
		name(name_), stream(stream_), time_stamp(time_stamp_), location(location_), timbre(timbre_), 
		content(content_), gender(gender_), speaker_id(speaker_id_),
		loudness(loudness_), pitch(0.), duration(duration_), intrinsic_duration(intrinsic_duration_)
//		time_stamp(Coordinator::get_time())
		{}

Auditory_sound::Auditory_sound(const Symbol& name_, const Symbol& stream_, long time_stamp_, GU::Point location_, const Symbol& timbre_, 
			const Symbol& content_, const Symbol& gender_, const Symbol& speaker_id_, double loudness_, double pitch_, long duration_, long intrinsic_duration_) :
		name(name_), stream(stream_), time_stamp(time_stamp_), location(location_), timbre(timbre_), 
		content(content_), gender(gender_), speaker_id(speaker_id_),
		loudness(loudness_), pitch(pitch_), duration(duration_), intrinsic_duration(intrinsic_duration_)
//		time_stamp(Coordinator::get_time())
		{}
...
Smart_Pointer<Auditory_sound> Auditory_sound::create(const Symbol& sound_name, const Symbol& stream_name, long time_stamp, GU::Point location, const Symbol& timbre,  
		...
		{return new Auditory_sound(sound_name, stream_name, time_stamp, location, timbre, Symbol(), Symbol(), Symbol(), loudness, duration, intrinsic_duration);}
...
Smart_Pointer<Auditory_sound> Auditory_sound::create(const Symbol& sound_name, const Symbol& stream_name, long time_stamp, GU::Point location, const Symbol& timbre, 
		...
		{return new Auditory_sound(sound_name, stream_name, time_stamp, location, timbre, content, gender, speaker_id, loudness, pitch, duration, intrinsic_duration);}
// smaller interface for speech sounds
Smart_Pointer<Auditory_sound> Auditory_sound::create(const Speech_word& word)
		{return new Auditory_sound(word.name, word.stream_name, word.time_stamp, word.location, Speech_c, word.content, word.speaker_gender, word.speaker_id, word.loudness, word.pitch, 0, word.duration);}
...
//	ot << name << ' ' << stream << ' ' << time_stamp << ' ' << timbre << ' '
	ot << name << ' ' << stream << ' ' << time_stamp << ' ' << location << ' ' << timbre << ' '
		...
		<< duration << ' ' << intrinsic_duration << ' ' << endl;
	properties.display_contents(ot);
}
...
void Auditory_store::insert_new(Smart_Pointer<Auditory_stream> obj_ptr)
{
...
Smart_Pointer<Auditory_stream> Auditory_store::get_stream_ptr(const Symbol& obj_name) const
{
...
// nothing is done to the store immediately when the sound stops
	...
	const Symbol& prop_value)
{
	...
	Smart_Pointer<Auditory_sound> obj_ptr = get_sound_ptr(obj_name);	// throws if not there
	// Symbol() will be returned if property was not there before
	Symbol old_prop_value = obj_ptr->get_property_value(prop_name);
	// do nothing if no change in property value
	if(prop_value == old_prop_value)
...
void Auditory_store::insert_new(Smart_Pointer<Auditory_sound> obj_ptr)
{
	const Symbol& obj_name = obj_ptr->get_name();
...
Smart_Pointer<Auditory_sound> Auditory_store::get_sound_ptr(const Symbol& obj_name) const
{
