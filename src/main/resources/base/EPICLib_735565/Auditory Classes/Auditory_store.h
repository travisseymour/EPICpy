...
#include "../Utility Classes/OPV_Containers.h"
...
class Auditory_stream : public Reference_Counted_Object {
public:
/* disabled 5/9/12
	static Smart_Pointer<Auditory_stream> create(const Symbol& name, GU::Point location, GU::Size size = GU::Size())
				{return new Auditory_stream(name, location, size);}
*/
	static Smart_Pointer<Auditory_stream> create(const Symbol& name, double pitch, double loudness, GU::Point location)
				{return new Auditory_stream(name, pitch, loudness, location);}
...
public:
	// for simple sounds
	static Smart_Pointer<Auditory_sound> create(const Symbol& sound_name, const Symbol& stream_name, long time_stamp, GU::Point location, const Symbol& timbre,  
			double loudness, long duration, long intrinsic_duration = 0);
	// fatter interface for speech sounds
	static Smart_Pointer<Auditory_sound> create(const Symbol& sound_name, const Symbol& stream_name, long time_stamp, GU::Point location, const Symbol& timbre, 
			const Symbol& content, const Symbol& gender, const Symbol& speaker_id, double loudness, double pitch, long duration, long intrinsic_duration);
	// smaller interface for speech sounds
	static Smart_Pointer<Auditory_sound> create(const Speech_word& word);
...
//	void set_next_link(const Symbol& next_link_)
//		{time_stamp = next_link_;}
...
protected:
	...
	Auditory_sound(const Symbol& name, const Symbol& stream, long time_stamp, GU::Point location, const Symbol& timbre, 
			const Symbol& content, const Symbol& gender, const Symbol& speaker_id, double loudness, long duration, long intrinsic_duration);
	Auditory_sound(const Symbol& name, const Symbol& stream, long time_stamp, GU::Point location, const Symbol& timbre, 
			const Symbol& content, const Symbol& gender, const Symbol& speaker_id, double pitch, double loudness, long duration, long intrinsic_duration);

private:
	// no default copy, assignment
	Auditory_sound(const Auditory_sound&);
	Auditory_sound& operator= (const Auditory_sound&);
...
//			const Symbol& content, const Symbol& speaker_gender, const Symbol& speaker_id, double loudness, long duration);
	...
	*/
	...
	Smart_Pointer<Auditory_stream> get_stream_ptr(const Symbol& obj_name) const;
...
protected:	
	...
	typedef std::map<Symbol, Smart_Pointer<Auditory_stream> > Stream_container_t;
	Stream_container_t streams;	// the stream storage
	typedef std::map<Symbol, Smart_Pointer<Auditory_sound> > Sound_container_t;
	...
	void insert_new(Smart_Pointer<Auditory_stream> obj_ptr);
	void insert_new(Smart_Pointer<Auditory_sound> obj_ptr);
	...
	// erase the specified object,throw an exception if not present
	void erase(Smart_Pointer<Auditory_stream> obj_ptr);
	void erase(Smart_Pointer<Auditory_sound> obj_ptr);
...
private:
	...
	// no default copy, assignment
	Auditory_store(const Auditory_store&);
	Auditory_store& operator= (const Auditory_store&);
};
