...
#include "../Architecture Classes/Cognitive_event_types.h"
#include "../Framework classes/Device_event_types.h"
#include "../Framework classes/Device_processor.h"
...
Smart_Pointer<Motor_action> Manual_action::create(Human_processor * human_ptr, 
...
{	
	...
	if(style == Keystroke_c) {
		arguments.pop_front();
		return new Manual_Keystroke_action(human_ptr, arguments, execute_when_prepared);
		}
	if(style == Hold_c) {
		arguments.pop_front();
		return new Manual_Hold_action(human_ptr, arguments, execute_when_prepared);
		}
	if(style == Release_c) {
		arguments.pop_front();
		return new Manual_Release_action(human_ptr, arguments, execute_when_prepared);
		}
	else if(style == Punch_c) {
		arguments.pop_front();
		return new Manual_Punch_action(human_ptr, arguments, execute_when_prepared);
		}
	else {
...
long Manual_Keystroke_action::prepare(long base_time)
{
	...
	const Manual_Keystroke_action * ptr = dynamic_cast<const Manual_Keystroke_action *>(get_processor_ptr()->get_previous_ptr());
...
long Manual_Keystroke_action::execute(long base_time)
{
	...
	long keystroke_execution_time = get_processor_ptr()->get_parameter_ptr("KLM_keystroke_execution_time")->get_value();
...
{
	long feature_preparation_time;
	const Manual_Hold_action * ptr = dynamic_cast<const Manual_Hold_action *>(get_processor_ptr()->get_previous_ptr());
	long feature_time = get_processor_ptr()->get_feature_time();
...
{
	double fluctuation = get_processor_ptr()->get_execution_fluctuation();
	long hold_execution_time = get_processor_ptr()->get_parameter_ptr("KLM_hold_release_execution_time")->get_value();
...
{
	long feature_preparation_time;
	const Manual_Release_action * ptr = dynamic_cast<const Manual_Release_action *>(get_processor_ptr()->get_previous_ptr());
	long feature_time = get_processor_ptr()->get_feature_time();
...
{
	double fluctuation = get_processor_ptr()->get_execution_fluctuation();
	long release_execution_time = get_processor_ptr()->get_parameter_ptr("KLM_hold_release_execution_time")->get_value();
...
{
	const Manual_Punch_action * ptr = dynamic_cast<const Manual_Punch_action *>(get_processor_ptr()->get_previous_ptr());
	int features;
