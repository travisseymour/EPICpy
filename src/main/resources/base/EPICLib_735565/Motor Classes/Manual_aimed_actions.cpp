...
#include "../Auditory Classes/Ear_processor.h"
...
#include "../Framework classes/Device_event_types.h"
...
Smart_Pointer<Motor_action> Manual_aimed_action::create(Human_processor * human_ptr, 
...
{	
	...
	arguments.pop_front();
	if(style == Ply_c) {
		return new Manual_Ply_action(human_ptr, arguments, execute_when_prepared);
		}
		...
		if(!arguments.empty())
			new_arguments.push_back(arguments.front());
		return new Manual_Point_action(human_ptr, new_arguments, execute_when_prepared);
		}
		...
		if(!arguments.empty())
			new_arguments.push_back(arguments.front());
		return new Manual_Click_on_action(human_ptr, new_arguments, execute_when_prepared);
		}
	else
		return 0;
}
...
long Manual_aimed_action::prepare(long base_time)
{
	...
	const Manual_aimed_action * exptr = dynamic_cast<const Manual_aimed_action *>(get_processor_ptr()->get_executing_ptr());
	...
	if(auditory_perceptual_store_ptr->is_sound_present(target)) {
		...
		target_device_name = human_ptr->get_Auditory_physical_store_ptr()->get_name_map().get_physical_name(target);
		}
	else {		
	...
	int features = 0;
	const Manual_aimed_action * ptr = dynamic_cast<const Manual_aimed_action *>(get_processor_ptr()->get_previous_ptr());
	if(ptr) {
...
//	movement_time += long((get_processor_ptr()->auto_home_to_mouse())); // right hand used on mouse or joystick
	...
	long subevent_time = get_processor_ptr()->get_parameter_ptr("Subevent_time")->get_value();
...
{
	long coefficient = get_processor_ptr()->get_parameter_ptr("Ply_coefficient")->get_value();
	long minimum_time = get_processor_ptr()->get_parameter_ptr("Ply_minimum_time")->get_value();
...
long Manual_Point_action::prepare(long base_time)
{
	...
	const Manual_Point_action * ptr = dynamic_cast<const Manual_Point_action *>(get_processor_ptr()->get_previous_ptr());
	if(ptr) {
	// dk 090807 - no r, theta feature programming time cost, but charge for a change in style or hand
...
double Manual_Point_action::determine_movement_time() const
{
	double movement_time;
	if(target_size == GU::Size()) {
		long KLM_point_time = get_processor_ptr()->get_parameter_ptr("KLM_point_time")->get_value();
		...
		long coefficient = (get_processor_ptr()->get_button_held()) ?
			get_processor_ptr()->get_parameter_ptr("Point_coefficient_button_held")->get_value()
			:
			get_processor_ptr()->get_parameter_ptr("Point_coefficient")->get_value();
		long minimum_time = get_processor_ptr()->get_parameter_ptr("Point_minimum_time")->get_value();
...
long Manual_Click_on_action::prepare(long base_time)
{
	...
	long feature_time = get_processor_ptr()->get_feature_time();
	const Manual_Click_on_action * ptr = dynamic_cast<const Manual_Click_on_action *>(get_processor_ptr()->get_previous_ptr());
	// dk 090807 - no r, theta feature programming time cost, but charge for a change in style or hand
	if(!ptr) {
...
long Manual_Click_on_action::generate_movement_events(long base_time)
{
	...
	long key_closure_time = get_processor_ptr()->get_parameter_ptr("Key_closure_time")->get_value();
	double fluctuation = get_processor_ptr()->get_execution_fluctuation();
