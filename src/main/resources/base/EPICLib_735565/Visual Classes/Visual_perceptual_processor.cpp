...
#include "../Framework classes/Visual_encoder_base.h"
...
void Visual_perceptual_processor::make_object_appear(const Symbol& name, GU::Point location, GU::Size size)
{
	...
	long visible_time = get_time() + visible_delay.get_value();
	schedule_event(new Visual_Change_Property_event(
...
void Visual_perceptual_processor::make_object_disappear(const Symbol& name)
{
	...
	long disappear_time = get_time() + appear_disappear_delay.get_value();
	...
	// arrange to remove the transient after its delay
	schedule_event(new Visual_Change_Property_event(
		disappear_time + onset_offset_decay_time.get_value(), get_human_ptr()->get_visual_perceptual_store_ptr(),
		name, Detection_c, Nil_c));
...
void Visual_perceptual_processor::set_object_location(const Symbol& name, GU::Point location)
{
	...
	schedule_event(new Visual_Change_Location_event(
		get_time() + location_delay.get_value(), get_human_ptr()->get_visual_perceptual_store_ptr(),
		name,location));
}
...
void Visual_perceptual_processor::set_object_size(const Symbol& name, GU::Size size)
{
	...
	schedule_event(new Visual_Change_Size_event(
		get_time() + size_delay.get_value(), get_human_ptr()->get_visual_perceptual_store_ptr(),
		name, size));
}
/*
...
void Visual_perceptual_processor::handle_event(const Visual_Delay_event * event_ptr)
{
	...
	// access the object
	Smart_Pointer<Visual_sensory_object> obj_ptr = get_sensory_object_ptr(object_name);
...
Smart_Pointer<Visual_sensory_object> Visual_perceptual_processor::get_sensory_object_ptr(const Symbol& object_name)
{
	Visual_store_object * p = get_human_ptr()->get_Visual_sensory_store_ptr()->get_object_ptr(object_name);
	Smart_Pointer<Visual_sensory_object> obj_ptr = dynamic_cast<Visual_sensory_object *>(p);
	if(!obj_ptr)
