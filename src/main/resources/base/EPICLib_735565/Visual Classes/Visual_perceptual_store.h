...
public:
	static Smart_Pointer<Visual_store_object> create(const Symbol& name, GU::Point location, GU::Size size)
				{return new Visual_perceptual_object(name, location, size);}
...
//	virtual void accept_event(const Stop_event *);
	...
	Smart_Pointer<Visual_perceptual_object> get_perceptual_object_ptr(const Symbol& name);
...
//	Parameter offset_decay_time;
	...
	void lose_object_from_memory(Smart_Pointer<Visual_perceptual_object> obj_ptr);
