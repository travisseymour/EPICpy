...
from epicpy2.utils.apputils import unpack_param_string

from epicpy2.epic.encoderpassthru import NullVisualEncoder, NullAuditoryEncoder
from epicpy2.epic.epicpyexception import EPICpyException
from epicpy2.epic.runinfo import RunInfo
from epicpy2.constants.stateconstants import *
from epicpy2.constants.emoji import *
from epicpy2.dialogs.loggingwindow import LoggingSettingsWin
...
from epicpy2.epiclib.epiclib import Model, Coordinator, Normal_out
from epicpy2.epiclib.epiclib import describe_parameters
from epicpy2.epiclib.epiclib import set_visual_encoder_ptr, set_auditory_encoder_ptr
...
class Simulation:
    ...
    def on_load_device(self, file: str = "", quiet: bool = False):
        ...
        if device_file:
            ...
            try:
                ...
                from epicpy2.utils.modulereloader import reset_module
            ...
            try:
                ...
                if isinstance(self.model, Model):
                    self.write(
                        f"{e_boxed_check} New Model() was successfully created with device."
                    )
                else:
                    raise EPICpyException(
                        f"{e_boxed_x} Error creating new Model() with device."
                    )
    ...
    def on_load_encoder(self, kind: str, file: str = "", quiet: bool = False):
        ...
        if encoder_file:
            ...
            for path in (
                self.visual_encoder_path_additions
                if kind == "Visual"
                else self.auditory_encoder_path_additions
            ):
                try:
                    sys.path.remove(path)
                except ValueError:
            ...
            try:
                ...
                if kind == "Visual":
                    exec(
                        f'self.visual_encoder = {encoder_file_p.stem}.VisualEncoder("{ul_name}", self)'
                    )
                else:
                    exec(
                        f'self.auditory_encoder = {encoder_file_p.stem}.AuditoryEncoder("{ul_name}", self)'
                    )
    ...
    def recompile_rules(self):
        ...
        if (
            self.rule_files
            and Path(self.rule_files[self.current_rule_index].rule_file).is_file()
        ):
            self.write(
    ...
    def choose_rules(self, files: Optional[list] = None) -> bool:
        ...
        if not files:
            ...
            raise ValueError(
                f"Unknown state in epicsimulation.choose_rules(): {mode=} {files=}"
            )
    ...
    def compile_rule(self, file: str = "", calm: bool = False) -> bool:
        ...
        try:
            ...
            if calm:
                self.write(
                    f"WARNING: Unable to compile ruleset file\n{rule_path.name}: {e}"
                )
            else:
                self.write(
                    emoji_box(
        ...
        if result:
            ...
            if calm:
                self.write(
                    f"WARNING: Unable to (re)compile ruleset file\n{rule_path.name}\n"
                )
            else:
                self.write(
                    emoji_box(
    ...
    def halt_simulation(self, reason: str = ""):
        ...
        if self.last_run_mode == "run_all":
            ...
            if self.current_rule_index < len(self.rule_files):
                ...
                if (
                    next_sim.device_file
                    and next_sim.device_file != prev_sim.device_file
                ) or next_sim.reload_device:
                    if not self.parent.on_load_device(
                        next_sim.device_file, auto_load_rules=False, quiet=True
                    ):
                        self.write(
