...
def setup_menu(window):
    ...
    window.recent_devices_menu = QMenu("Load Recent Device", window)
    window.recent_devices_menu.setFont(window.font())
    file_menu.addMenu(window.recent_devices_menu)
    file_menu.addSeparator()
    ...
    window.actionRun_Normal = QAction("Run", window)
    window.actionRun_Normal.setEnabled(False)
    run_menu.addAction(window.actionRun_Normal)
    ...
    window.actionRestore_Info_Output = QAction("Restore Info Output", window)
    ...
    windows_menu.addAction(window.actionRestore_Info_Output)
    ...
    window.actionProcessViewerTool = QAction("Process Viewer", window)
    tool_menu.addAction(window.actionProcessViewerTool)
...
def setup_menu_connections(window):
    ...
    window.actionUnload_Auditory_Encoder.triggered.connect(partial(window.simulation.on_unload_encoder, kind="Auditory"))

    # Run menu actions
    window.actionAllow_Parallel_Runs.toggled.connect(window.toggle_allow_parallel_runs)
    window.actionRun_Settings.triggered.connect(window.show_run_settings)

    window.actionRun_Normal.triggered.connect(lambda _: window.run_normal())
    window.actionRun_Normal.setShortcut("Ctrl+R")
    ...
    window._refresh_recent_devices_menu()
    ...
    window.actionRestore_Info_Output.triggered.connect(partial(window.restore_ui_component, component="info"))
    ...
    # Tools menu
    window.actionRuleFlowTool.setEnabled(False)  # Enabled dynamically when normalTextOutput has cycle data
    window.actionSchematicTool.setEnabled(False)
    window.actionProcessViewerTool.setEnabled(False)  # Enabled dynamically when traceTextOutput has Human data
    ...
    window.actionProcessViewerTool.triggered.connect(partial(window.start_tool, tool_name="process_viewer"))
