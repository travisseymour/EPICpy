...
from qtpy.QtCore import QPoint, QPointF, QRect, QRectF, QSize, Qt
...
from qtpy.QtWidgets import QGraphicsTextItem, QMainWindow

from epicpy.utils import config
from epicpy.utils.app_utils import Point, Rect, Size, conditional_dataclass
from epicpy.views.epic_colors import epic_colors as colors
...
class VisualViewWin(QMainWindow):
    def __init__(self, view_type: str, view_title: str):
        ...
        self.enabled = True
        self.can_draw = True
        self.batch_mode = False  # When True, suppress update() calls until explicitly triggered
    ...
    def set_batch_mode(self, enabled: bool):
        """Enable/disable batch mode. When enabled, update() calls are suppressed."""
    ...
    def set_needs_display(self):
        # In batch_mode, suppress update() calls - they will be triggered periodically
        if not self.batch_mode:
            self.update()

    def force_display(self):
        """Force an update regardless of batch_mode - used for periodic refresh."""
        self.can_draw = True
        self.update()
    ...
    def create_new_object(
        ...
        if object_name not in self.objects:
            ...
            #             _o = VisualObject(
            #                 kind=o.kind, name=o.name, location=str(_location), size=str(_size),
            #                 property=o.property.toDict() if isinstance(o.property, Munch) else {}
            #             )
    ...
    def draw_info_overlay(self, painter: QPainter):
        ...
        # draw view info
        s = f"{self.width() / self.scale:0.2f} X {self.height() / self.scale:0.2f} DVA, (0 , 0), {self.scale} p/DVA"
    ...
    def shape_cache(shape, x, y, w, h) -> QPainterPath:
        ...
        if shape == "ellipse":
            ...
            k = QPointF(j.x(), c.y())
            ll = QPointF(a.x(), c.y())
            path.addPolygon(QPolygonF((a, b, c, d, e, f, g, _h, i, j, k, ll, a)))
        elif shape == "arc":
            box = QRect(x, y, w, h)
            path = QPainterPath()
            # old approach
            ...
            k = QPointF(j.x(), c.y())
            ll = QPointF(a.x(), k.y() + gap)
            path.addPolygon(QPolygonF((a, b, c, d, e, f, g, h, i, j, k, ll, a)))
            ...
            box = QRectF(x, y, w, h)
            cpx = int(box.center().x())
            cpy = int(box.bottomRight().y() + box.height())
            ...
            box = QRectF(x, y, w, h)
            cpx = int(box.center().x())
            cpy = int(box.topRight().y() - box.height())
