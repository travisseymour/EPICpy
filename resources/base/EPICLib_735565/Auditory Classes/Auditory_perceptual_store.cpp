...
Smart_Pointer<Auditory_sound> Auditory_perceptual_sound::create(const Speech_word& word)
{
	{return new Auditory_perceptual_sound(word.name, word.stream_name, word.time_stamp, word.location, Speech_c, word.content, word.speaker_gender, word.speaker_id, word.loudness, word.pitch, 0, word.duration);}
}
...
Smart_Pointer<Auditory_perceptual_sound> Auditory_perceptual_store::get_perceptual_object_ptr(const Symbol& sound_name)
{
	Auditory_sound * p = get_sound_ptr(sound_name);
	Smart_Pointer<Auditory_perceptual_sound> obj_ptr = dynamic_cast<Auditory_perceptual_sound *>(p);
	if(!obj_ptr)
...
void Auditory_perceptual_store::handle_event(const Auditory_Stream_Destroy_event * event_ptr)
{
	...
	schedule_event(new Auditory_Stream_Erase_event( 
		get_time() + unsupported_decay_delay.get_value(), this,
...
void Auditory_perceptual_store::handle_event(const Auditory_Stream_Erase_event * event_ptr)
{
	...
	Smart_Pointer<Auditory_stream> obj_ptr = get_stream_ptr(event_ptr->stream_name);
...
void Auditory_perceptual_store::handle_event(const Auditory_Sound_Stop_event * event_ptr)
{
	...
	Smart_Pointer<Auditory_perceptual_sound> obj_ptr = get_perceptual_object_ptr(event_ptr->sound_name);
...
void Auditory_perceptual_store::handle_event(const Auditory_Sound_Erase_event * event_ptr)
{
	...
	switch(obj_ptr->get_state()) {
		case Auditory_perceptual_sound::UNSUPPORTED: {
			...
			// schedule an erasure event for later
			schedule_event(new Auditory_Sound_Erase_event( 
				get_time() + retained_decay_delay.get_value(), this, event_ptr->sound_name));
...
void Auditory_perceptual_store::lose_sound_from_memory(Smart_Pointer<Auditory_perceptual_sound> obj_ptr)
{
	...
	Name_map& name_map = get_human_ptr()->get_Auditory_physical_store_ptr()->get_name_map();
...
{
	// access the object
	Smart_Pointer<Auditory_sound> obj_ptr = get_sound_ptr(event_ptr->sound_name);
	...
	if(event_ptr->property_value == Nil_c && old_value != Nil_c && event_ptr->property_name != Detection_c) {
		...
		long disappearing_time = get_time() + property_decay_time_delay.get_value();
