...
public:
	Motor_processor(const std::string& name_, Human_processor * human_ptr_, bool generate_signals_ = true) :
		...
		preparing_action(0), previous_prepared_action(0),ready_action(0),
		executing_action(0),movement_completion_time(0),
// dk 090807
...
//		{return waiting_action;}
	...
	long get_feature_time()
		{return feature_time.get_value();}
	long get_initiation_time()
		{return initiation_time.get_value();}
	long get_burst_time()
		{return burst_time.get_value();}
	long get_efferent_deletion_delay()
		{return efferent_deletion_delay.get_value();}
	double get_execution_fluctuation()
		{return execution_fluctuation.get_double_value();}
	const Motor_action * get_previous_ptr() const
		{return previous_prepared_action;}
	const Motor_action * get_executing_ptr() const
		{return executing_action;}
...
//	Motor_action * waiting_action;
	Smart_Pointer<Motor_action> preparing_action;
	Smart_Pointer<Motor_action> previous_prepared_action;
	Smart_Pointer<Motor_action> ready_action;
	Smart_Pointer<Motor_action> executing_action;
