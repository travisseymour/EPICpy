...
Smart_Pointer<Motor_action> Vocal_action::create(Human_processor * human_ptr, 
...
{	
	...
	arguments.pop_front();
	if(style == Speak_c) {
		return new Vocal_Speak_action(human_ptr, arguments, true, execute_when_prepared);
		}
	if(style == Subvocalize_c) {
		return new Vocal_Speak_action(human_ptr, arguments, false, execute_when_prepared);
		}
	else
		return 0;
}
...
long Vocal_Speak_action::prepare(long base_time)
{
	...
	const Vocal_Speak_action * ptr = dynamic_cast<const Vocal_Speak_action *>(get_processor_ptr()->get_previous_ptr());
...
{
	long syllable_time = get_processor_ptr()->get_parameter_ptr("Syllable_time")->get_value();
