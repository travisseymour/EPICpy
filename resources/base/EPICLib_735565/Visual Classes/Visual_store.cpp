...
GU::Point Visual_store::set_object_location(const Symbol& obj_name, GU::Point location)
{
	...
	Smart_Pointer<Visual_store_object> obj_ptr = get_object_ptr(obj_name);	// throws if not there
...
Symbol_list_t Visual_store::find_all (const Symbol_list_t& pv_list) const
{
	...
	for (Store_container_t::const_iterator obj_iter = objects.begin(); 
		...
		Smart_Pointer<Visual_store_object> obj_ptr = obj_iter->second;
...
void Visual_store::insert_new(Smart_Pointer<Visual_store_object> obj_ptr)
{
	...
	if(it != objects.end()) {
/*		Smart_Pointer<Visual_store_object> orig_ptr = it->second;
...
Smart_Pointer<Visual_store_object> Visual_store::get_object_ptr(const Symbol& obj_name) const
{
