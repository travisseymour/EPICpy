...
public:
	Eye_processor(Human_processor * human_ptr_) :
		...
		reflex_enabled(false), reflex_on(false), involuntary_movement_in_progress(false), 
...
private:
	...
	Smart_Pointer<Visual_store_object> closest_object_ptr;	// current object closest to point of fixation
...
//	Parameter disappearance_delay;
	...
	typedef std::map<Symbol, Smart_Pointer<Availability> > Availability_map_t;
	Availability_map_t availabilities;
	Smart_Pointer<Availability> default_availability;
...
//	void finish_make_object_appear(const Symbol& physical_name, GU::Point location, GU::Size size);
	...
	bool update_eccentricity_and_visibility(Smart_Pointer<Visual_store_object> obj_ptr);
	bool update_location_information(Smart_Pointer<Visual_store_object> obj_ptr);
	void send_eccentricity_update(const Symbol& psychological_name, double ecc);
	void send_property_availability_update(Smart_Pointer<Visual_store_object> obj_ptr, 
...
//		const Symbol& prop_name, const Symbol& prop_value);
	...
	void update_all_properties(Smart_Pointer<Visual_store_object> obj_ptr);
