...
Smart_Pointer<Visual_perceptual_object> Visual_perceptual_store::get_perceptual_object_ptr(const Symbol& object_name)
{
	Visual_store_object * p = get_object_ptr(object_name);
	Smart_Pointer<Visual_perceptual_object> obj_ptr = dynamic_cast<Visual_perceptual_object *>(p);
	if(!obj_ptr)
...
void Visual_perceptual_store::handle_event(const Visual_Appear_event * event_ptr)
{
	...
	// if it is already present, it is being reinstated, so we need to change its state to present and Status Property to Visible
		...
		Smart_Pointer<Visual_perceptual_object> obj_ptr = get_perceptual_object_ptr(event_ptr->name);
...
/*	// save the properties of the object
	Smart_Pointer<Visual_store_object> obj_ptr = get_object_ptr(event_ptr->object_name);
...
*/
	...
	Smart_Pointer<Visual_perceptual_object> obj_ptr = get_perceptual_object_ptr(event_ptr->object_name);
	...
	long first_erase_time = get_time() + unsupported_object_decay_time.get_value();
	...
	switch(obj_ptr->get_state()) {
		case Visual_perceptual_object::UNSUPPORTED: {
			...
			*/
			...
			// schedule an erasure event for later
			long second_erase_time = get_time() + retained_object_decay_time.get_value();
...
void Visual_perceptual_store::lose_object_from_memory(Smart_Pointer<Visual_perceptual_object> obj_ptr)
{
...
bool Visual_perceptual_store::update_retained_objects()
{
	...
	typedef list<Smart_Pointer<Visual_perceptual_object> > Perceptual_object_ptr_list_t;
...
// KLUDGE KLUDGE KLUDGE
	...
	if(new_value == Nil_c && old_value != Nil_c && event_ptr->property_name != Detection_c) {
		...
		long disappearing_time = get_time() + property_decay_time.get_value();
