...
#include "../Motor Classes/Invol_ocular_actions.h"
#include "../Motor Classes/Invol_ocular_processor.h"
#include "../Motor Classes/Ocular_processor.h"
...
void Eye_processor::setup()
{
	...
	availabilities[Visible_c] =
		new Periphery_std_zone_availability(physical_store, Visible_c);
	availabilities[Position_c] = 
		new Relation_std_zone_availability(physical_store, Position_c);
	availabilities[Vposition_c] = 
		new Relation_std_zone_availability(physical_store, Vposition_c);
	availabilities[Hposition_c] = 
		new Relation_std_zone_availability(physical_store, Hposition_c);
	availabilities[In_row_c] = 
		new Relation_std_zone_availability(physical_store, In_row_c);
	availabilities[In_col_c] = 
		new Relation_std_zone_availability(physical_store, In_col_c);
	availabilities[Orientation_c] = 
		new Relation_std_zone_availability(physical_store, Orientation_c);
	availabilities[Leader_c] = 
		new Relation_std_zone_availability(physical_store, Orientation_c);
	availabilities[Above_c] = 
		new Relation_std_zone_availability(physical_store, Above_c);
	availabilities[Below_c] = 
		new Relation_std_zone_availability(physical_store, Below_c);
	availabilities[Left_of_c] = 
		new Relation_std_zone_availability(physical_store, Left_of_c);
	availabilities[Right_of_c] = 
		new Relation_std_zone_availability(physical_store, Right_of_c);
	availabilities[Inside_c] = 
		new Relation_std_zone_availability(physical_store, Inside_c);
	availabilities[In_center_of_c] = 
		new Relation_std_zone_availability(physical_store, In_center_of_c);
	availabilities[Outside_c] = 
		new Relation_std_zone_availability(physical_store, Outside_c);
	availabilities[Placed_on_c] = 
		new Relation_std_zone_availability(physical_store, Placed_on_c);
	availabilities[Depth_c] = 
		new Relation_std_zone_availability(physical_store, Depth_c);
	availabilities[Distance_c] = 
		new Relation_std_zone_availability(physical_store, Distance_c);
	availabilities[Color_c] = 
		new Color_std_zone_availability(physical_store);
	availabilities[Shape_c] = 
		new Shape_std_zone_availability(physical_store);
	availabilities[Text_c] = 
		new Text_std_zone_availability(physical_store);
//	availabilities[Size_c] = 
//		new Fovea_std_zone_availability(physical_store, Size_c);
	availabilities[Encoded_size_c] = 
		new Fovea_std_zone_availability(physical_store, Encoded_size_c);
	default_availability = 
		new Fovea_std_zone_availability(physical_store, Default_c);
...
void Eye_processor::set_parameter(const Parameter_specification& param_spec)
{
	if(param_spec.parameter_name == "Availability") {
		Visual_physical_store& physical_store = *(get_human_ptr()->get_Visual_physical_store_ptr());
		Smart_Pointer<Availability> p = Availability::create(physical_store, param_spec);
...
//		Smart_Pointer<Availability> a_ptr = it->second;
...
bool Eye_processor::update_eccentricity_and_visibility(Smart_Pointer<Visual_store_object> obj_ptr)
{
	...
	Smart_Pointer<Availability> a_ptr = it_availability->second;
	double x = eccentricity_fluctuation.get_double_value();
	bool visible = a_ptr->available(obj_ptr, x);
...
void Eye_processor::make_object_appear(const Symbol& physical_name, GU::Point location, GU::Size size)
{
	...
	Smart_Pointer<Visual_store_object> obj_ptr = get_human_ptr()->get_Visual_physical_store_ptr()->get_object_ptr(physical_name);
...
//	bool new_visibility = obj_ptr->is_visible();
	...
	if(new_visibility) {
		...
		long appearance_time = get_time() + appearance_disappearance_delay.get_value();
...
//		involuntary_movement_in_progress = true;
		...
		last_invol_ocular_command_time = get_time() + appearance_disappearance_delay.get_value() + inform_invol_ocular_delay.get_value();
...
//	make_object_disappear(event_ptr->object_name);
	...
	schedule_event(new Visual_Disappear_event( 
		get_time() + appearance_disappearance_delay.get_value(), get_human_ptr()->get_visual_sensory_store_ptr(),
...
void Eye_processor::make_object_disappear(const Symbol& physical_name)
{
	...
	*/
	...
	if(voluntary_saccade_underway || involuntary_saccade_underway) {
		...
		long disappearance_time = get_time() + appearance_disappearance_delay.get_value();
		schedule_event(new Visual_Disappear_event( 
...
//		return;
	...
	const Symbol& psychological_name = name_map.get_psychological_name(physical_name);
	schedule_event(new Visual_Change_Location_event(
		get_time() + location_delay.get_value(), get_human_ptr()->get_visual_sensory_store_ptr(),
...
//		involuntary_movement_in_progress = true;
		Smart_Pointer<Motor_action> action_ptr = new Invol_ocular_Saccade_action(get_human_ptr(), psychological_name);
		last_invol_ocular_command_time = get_time() + location_delay.get_value() + inform_invol_ocular_delay.get_value();
		action_ptr->dispatch(last_invol_ocular_command_time);
...
void Eye_processor::set_object_size(const Symbol& physical_name, GU::Size size)
{
	...
	if(visible) {
		...
		schedule_event(new Visual_Change_Size_event(
			get_time() + size_delay.get_value(), get_human_ptr()->get_visual_sensory_store_ptr(),
...
void Eye_processor::set_object_property(const Symbol& physical_name, const Symbol& prop_name, const Symbol& prop_value)
{
	...
	Smart_Pointer<Availability> a_ptr = it_availability->second;
	double x = eccentricity_fluctuation.get_double_value();
	bool available = a_ptr->available(obj_ptr, x);
	...
	Symbol new_value = change_related_object_name(prop_name, prop_value);
	
	// go ahead and send the property forward
	long delay = a_ptr->delay(obj_ptr, property_time_fluctuation.get_double_value());
...
bool Eye_processor::update_location_information(Smart_Pointer<Visual_store_object> obj_ptr)
{
	bool old_visibility = obj_ptr->is_visible();
	// - dk 082909
//	obj_ptr->update_eccentricity_and_visibility(eye_location, standard_peripheral_radius);
//	bool new_visibility = obj_ptr->is_visible();
	...
	if(new_visibility) {
		if(!old_visibility) {	// came into view or appeared
			...
			// the physical name is now in use for an object that is present
			name_map.set_physical_name_in_use(physical_name, true);
			long appearance_delay_time = appearance_disappearance_delay.get_value();
			schedule_event(new Visual_Appear_event(
...
//	else if(old_visibility || old_was_visible) {
		...
		send_eccentricity_update(psychological_name, obj_ptr->get_eccentricity());
		schedule_event(new Visual_Disappear_event( 
			get_time() + appearance_disappearance_delay.get_value(), get_human_ptr()->get_visual_sensory_store_ptr(), psychological_name));
		return true;
		}
...
void Eye_processor::send_eccentricity_update(const Symbol& psychological_name, double ecc)
{
	...
	const Symbol& eccentricity_zone_value = (ecc <= standard_fovea_radius) ?
		Fovea_c : Periphery_c;
	schedule_event(new Visual_Change_Property_event(get_time() + eccentricity_delay.get_value(), 
...
void Eye_processor::update_all_properties(Smart_Pointer<Visual_store_object> obj_ptr)
{
	// access the object
...
*/
void Eye_processor::send_property_availability_update(Smart_Pointer<Visual_store_object> obj_ptr, 
...
void Eye_processor::update_closest_object()
{
	...
	const Visual_store::Store_object_ptr_list_t& object_ptrs = visual_physical_store_ptr->get_object_ptr_list();
	for(Visual_store::Store_object_ptr_list_t::const_iterator it = object_ptrs.begin(); it != object_ptrs.end(); ++it) {
		Smart_Pointer<Visual_store_object> obj_ptr = *it;
...
//		involuntary_movement_in_progress = true; // this gets reset for either a voluntary or involuntary movement
		...
		Smart_Pointer<Motor_action> action_ptr = new Invol_ocular_Smooth_move_action(
				get_human_ptr(), name_map.get_psychological_name(closest_object_ptr->get_name()));
		last_invol_ocular_command_time = get_time() + location_delay.get_value() + inform_invol_ocular_delay.get_value();
		action_ptr->dispatch(last_invol_ocular_command_time);
		if(get_trace() && Trace_out)
			Trace_out << processor_info() << "Invol_ocular_Smooth_move_action commanded for " << last_invol_ocular_command_time << endl;
...
//		involuntary_movement_in_progress = true; // this gets reset for either a voluntary or involuntary movement
		// tell involuntary ocular processor to do an involuntary saccade to the closest object
		Smart_Pointer<Motor_action> action_ptr = new Invol_ocular_Saccade_action(
				get_human_ptr(), name_map.get_psychological_name(closest_object_ptr->get_name()));
		last_invol_ocular_command_time = get_time() + location_delay.get_value() + inform_invol_ocular_delay.get_value();
		action_ptr->dispatch(last_invol_ocular_command_time);
		if(get_trace() && Trace_out)
